...
const HighlightedText: React.FC<{ text: string; pattern: string; flags: string; isDark: boolean }> = ({ text, pattern, flags, isDark }) => {
    if (!pattern || !text) {
        return (
            <pre className={`font-mono text-[13px] leading-[1.6] whitespace-pre-wrap break-words ${isDark ? 'text-[#c5c5d8]' : 'text-[#374151]'}`}>
                {text || <span className={isDark ? 'text-[#3d3d6b]' : 'text-[#d1d5db]'}>Your test string will appear here with matches highlighted…</span>}
            </pre>
        )
    }
...
const FlagToggle: React.FC<{ flag: string; label: string; active: boolean; onToggle: () => void; isDark: boolean }> = ({ flag, label, active, onToggle, isDark }) => (
    <button
        ...
        onClick={onToggle}
        title={label}
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono font-semibold transition-all duration-150 cursor-pointer border
            ${active
                ? 'bg-[#4f6ef7] border-[#4f6ef7] text-white shadow shadow-[#4f6ef730]'
                : isDark
                    ? 'bg-[#1e1e35] border-[#2a2a45] text-[#6b7280] hover:border-[#4f6ef7] hover:text-[#9595b4]'
                    : 'bg-white border-[#e5e7eb] text-[#9ca3af] hover:border-[#4f6ef7] hover:text-[#4f6ef7]'
            }`}
    >
        <span className="italic">{flag}</span>
        <span className={`text-[10px] font-sans font-normal ${active ? 'text-[#c7d2fe]' : isDark ? 'text-[#4b5563]' : 'text-[#d1d5db]'}`}>{label}</span>
    </button>
)
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="ml-auto flex gap-2">
                        ...
                        <button id="regex-clear-btn" onClick={handleClear}
                            className={`px-3 py-1.5 rounded-lg text-xs font-mono transition-all duration-150 cursor-pointer border
                                ${isDark ? 'bg-[#1e1e35] border-[#2a2a45] text-[#6b7280] hover:bg-[#252545]' : 'bg-white border-[#e5e7eb] text-[#9ca3af] hover:bg-[#f3f4f6]'}`}>
                            Clear
                        </button>
                    </div>
                </div>
                    ...
                    <div className={`rounded-md border overflow-hidden ${isDark ? 'border-[#2a2a45]' : 'border-[#e5e7eb]'}`}>
                        {matches.map((m, i) => (
                            <div key={i} className={`flex items-start gap-4 px-4 py-3 font-mono text-[12px]
                                ...
                                <span className={`shrink-0 text-[10px] px-2 py-0.5 rounded font-bold ${isDark ? 'bg-[#1e1e35] text-[#6b7280]' : 'bg-[#f3f4f6] text-[#9ca3af]'}`}>
                                    #{i + 1}
                                </span>
                                <div className="flex flex-col gap-1 min-w-0">
                                    <div className="flex items-center gap-3">
                                        <span className={`${isDark ? 'text-[#a5b4fc]' : 'text-[#4338ca]'} font-semibold`}>"{m.fullMatch}"</span>
                                        <span className={`text-[11px] ${isDark ? 'text-[#4b5563]' : 'text-[#9ca3af]'}`}>@ index {m.index}</span>
                                    </div>
                                    {m.groups.length > 0 && (
                                        <div className="flex flex-wrap gap-2">
                                            {m.groups.map((g, gi) => (
                                                <span key={gi} className={`flex items-center gap-1 text-[11px] px-2 py-0.5 rounded border
                                                    ${isDark ? 'bg-[#1a1a2e] border-[#2a2a45] text-[#9595b4]' : 'bg-[#f3f4f6] border-[#e5e7eb] text-[#6b7280]'}`}>
                                                    <span className={isDark ? 'text-[#6b7280]' : 'text-[#9ca3af]'}>{g.key}:</span>
                                                    <span className={isDark ? 'text-[#c5c5d8]' : 'text-[#374151]'}>"{g.value}"</span>
                                                </span>
                                            ))}
                                        </div>
                                    )}
